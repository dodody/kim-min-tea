...
// getData 가 반환하는 값이 두가지 일 경우, 제네릭을 사용할 것
// 입력이 n개 일 때, 출력도 n개 인것을 말하는 것.
...
function makeFeeds(feeds: tNewsFeed[]): tNewsFeed[] {
...
function newsDetail(): void {
  ...
  const newsContent = getData<tNewsDetail>(CONTENT_URL.replace("@id", id));
  let template = `
...
function makeComment(comments: tComment[]): string {
  const commentString = [];
